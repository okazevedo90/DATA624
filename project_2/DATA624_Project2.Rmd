---
title: "DATA624 Project 2"
author: "Olivia Azevedo, Ahmed Hassan, Ariba Razzaq, Tatyana Titova"
date: "`r Sys.Date()`"
output:
  html_document:
    code_folding: "hide"
    theme: flatly
    toc: true
    toc_float:
      collapsed: true
---


```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)

library(fpp3)
library(kableExtra)
library(ggplot2)
library(rio)
library(skimr)
library(zoo)
library(writexl)
library(readxl)
library(caret)
library(dplyr)
library(corrplot)
library(nnet)
library(randomForest)
library(fastDummies)
library(modEvA)

knit_table <- function(df, caption='', position = 'left', color = '#2d3c52') {
  kbl(df, format = "html", escape = FALSE, caption = caption) |>
    kable_styling(
      full_width = F,
      position = position,
      bootstrap_options = c("striped")) |>
    row_spec(0, bold = T,
             color = "white",
             background = color,
             extra_css = paste("border: 2px solid", color, ";")) |>
    row_spec(dim(df)[1], extra_css = paste("border-bottom: 2px solid", color, ";")) |>
    column_spec(1, extra_css = paste("border-left: 2px solid", color, ";")) |>
    column_spec(dim(df)[2], extra_css = paste("border-right: 2px solid", color, ";")) |>
    scroll_box(width = "100%", height = ifelse(((nrow(df)+1)*45)+50 > 450, "450px", paste0(((nrow(df)+1)*45)+50, "px")), box_css = "border: 1px solid #FFFFFF;")
}
```

# Import Data
```{r}
# import data from github
train_data_url = "https://github.com/okazevedo90/DATA624/raw/refs/heads/main/project_2/StudentData.xlsx"
test_data_url = "https://github.com/okazevedo90/DATA624/raw/refs/heads/main/project_2/StudentEvaluation.xlsx"

train_data = import(train_data_url)
test_data = import(test_data_url)

head(train_data)
head(test_data)
```

# EDA

## Initial Data Exploration
```{r}
# =========================================================
# INITIAL DATA EXPLORATION 
# =========================================================

# View dataset structure
str(train_data)

# Summary statistics
summary(train_data)

# Dataset dimensions
dim(train_data)

# Detailed overview
skim(train_data)
```

## Check Missing Value
```{r}
# =========================================================
# CHECK MISSING VALUES
# =========================================================

# Count missing values in each column
missing_values <- colSums(is.na(train_data))

# Display missing value counts
missing_values

# Create missing value table
missing_table <- data.frame(
  Variable = names(missing_values),
  Missing_Count = missing_values
)

missing_table
```

## Histograms for Numeric Variables
```{r}
# =========================================================
#  HISTOGRAMS FOR NUMERIC VARIABLES
# =========================================================

# Select numeric columns only
numeric_cols <- train_data %>%
  select(where(is.numeric))

# Create histograms for all numeric variables
for(col in names(numeric_cols)){
  print(
    ggplot(train_data, aes(x = .data[[col]])) +
      geom_histogram(
        bins = 30,
        fill = "skyblue",
        color = "black"
      ) +
      labs(
        title = paste("Histogram of", col),
        x = col,
        y = "Frequency"
      ) +
      theme_minimal()
  )
}
```

## Outlier Detection
```{r}

# =========================================================
#  BOXPLOTS FOR OUTLIER DETECTION
# =========================================================

for(col in names(numeric_cols)){
  print(
    ggplot(train_data, aes(y = .data[[col]])) +
      geom_boxplot(fill = "tomato") +
      labs(
        title = paste("Boxplot of", col),
        y = col
      ) +
      theme_minimal()
  )
}
```

## Correlation Analysis
```{r}
# =========================================================
#  CORRELATION ANALYSIS
# =========================================================

# Compute correlation matrix
cor_matrix <- cor(
  numeric_cols,
  use = "pairwise.complete.obs"
)

# Correlation heatmap
corrplot(
  cor_matrix,
  method = "color",
  type = "upper",
  tl.cex = 0.7
)
```

## Scatterplots 
```{r}
# =========================================================
#  SCATTERPLOTS VS PH
# =========================================================

# Remove PH from predictor list
predictor_cols <- setdiff(names(numeric_cols), "PH")

# Scatterplots against PH
for(col in predictor_cols){
  print(
    ggplot(train_data, aes(x = .data[[col]], y = PH)) +
      geom_point(alpha = 0.6, color = "blue") +
      geom_smooth(method = "lm", se = FALSE, color = "red") +
      labs(
        title = paste(col, "vs PH"),
        x = col,
        y = "PH"
      ) +
      theme_minimal()
  )
}
```

# Data Prep

## Fearture Engineering
```{r}
# =========================================================
#  FEATURE ENGINEERING
# =========================================================

# Convert Brand Code to categorical variable
if("Brand Code" %in% names(train_data)) {

  train_data$`Brand Code` <- as.character(
    train_data$`Brand Code`
  )

  train_data$`Brand Code`[
    is.na(train_data$`Brand Code`)
  ] <- "Unknown"

  train_data$`Brand Code` <- as.factor(
    train_data$`Brand Code`
  )

  test_data$`Brand Code` <- as.character(
    test_data$`Brand Code`
  )

  test_data$`Brand Code`[
    is.na(test_data$`Brand Code`)
  ] <- "Unknown"

  test_data$`Brand Code` <- factor(
    test_data$`Brand Code`,
    levels = levels(train_data$`Brand Code`)
  )
}

# Transform Brand Code into Dummy Variables
train_data = dummy_cols(train_data, select_columns = "Brand Code", remove_selected_columns = TRUE)
test_data = dummy_cols(test_data, select_columns = "Brand Code", remove_selected_columns = TRUE)
```


## Seperate Target Variable
```{r}
# =========================================================
# SEPARATE TARGET VARIABLE
# =========================================================

# PH is the prediction target
y <- train_data$PH

# Remove PH from predictors
x <- train_data %>%
  select(-PH)
```

## Remove Near-Zero Variance Variables
```{r}
# =========================================================
#  REMOVE NEAR-ZERO VARIANCE VARIABLES
# =========================================================

# Identify low-information predictors
nzv <- nearZeroVar(x)

# Remove them if any exist
if(length(nzv) > 0){

  x <- x[, -nzv]
}

# Match columns in test dataset
test_processed <- test_data[, names(x)]
```

## Remove Highly Corrrelated Variables
```{r}
# =========================================================
#  REMOVE HIGHLY CORRELATED VARIABLES
# =========================================================

# Keep numeric variables only
numeric_x <- x %>%
  select(where(is.numeric))

# Correlation matrix
cor_matrix <- cor(
  numeric_x,
  use = "pairwise.complete.obs"
)

# Identify highly correlated variables
highCorr <- findCorrelation(
  cor_matrix,
  cutoff = 0.90
)

# Remove correlated predictors
if(length(highCorr) > 0){

  highCorr_names <- colnames(numeric_x)[highCorr]

  x <- x[, !(names(x) %in% highCorr_names)]

  test_processed <- test_processed[, names(x)]
}
```



## Data Preprocessing
```{r}
# =========================================================
#  PREPROCESSING
# =========================================================

# Apply:
# 1. Median imputation
# 2. Centering
# 3. Scaling

preprocess_model <- preProcess(
  x,
  method = c(
    "medianImpute",
    "center",
    "scale"
  )
)

# Apply preprocessing to training data
x_processed <- predict(
  preprocess_model,
  x
)

# Apply SAME preprocessing to test data
test_processed <- predict(
  preprocess_model,
  test_processed
)
```

## Final Data Checks
```{r}
# =========================================================
#  FINAL DATASET CHECKS
# =========================================================

# Check for remaining missing values
colSums(is.na(x_processed))

# Final dataset dimensions
dim(x_processed)

# Preview processed dataset
head(x_processed)
```


## Create and Export Final Modeling Dataset
```{r}
# =========================================================
#  CREATE FINAL MODELING DATASET
# =========================================================

# Combine target variable with processed predictors
final_train_data <- cbind(
  PH = y,
  x_processed
)

# Preview final dataset
head(final_train_data)
```


```{r}
# =========================================================
#  EXPORT FINAL DATASETS
# =========================================================

# Export processed training dataset
write_xlsx(
  final_train_data,
  "Processed_Training_Data.xlsx"
)

# Export processed test dataset
write_xlsx(
  test_processed,
  "Processed_Test_Data.xlsx"
)
```


**UPDATE - AHMED**
```{r}
final_train_data <- final_train_data[!is.na(final_train_data$PH), ]
```


## Split Data
```{r}
set.seed(123)

# split data into train and test sets
trainingRows <- createDataPartition(final_train_data$PH, p = .80, list= FALSE)

# train 
train <- final_train_data[trainingRows, ] 

# test
test = final_train_data[-trainingRows, ]
```

# Model Development {.tabset}

## Linear Regression
```{r}
set.seed(123)

# 10-fold cross-validation
ctrl <- trainControl(method = "cv", number = 5)

# fit model
lmModel <- train(PH ~ .,
    data = train,
    method = "lm", trControl = ctrl) 

# review performance metrics
lm_pred <- predict(lmModel,  test)
lm_results = postResample(pred = lm_pred, obs = test$PH)
lm_results_df = rownames_to_column(data.frame(lm_results), var = "Metric")
colnames(lm_results_df) = c('Metric', 'Score')
knit_table(lm_results_df, 'Linear Regression Performance')
```

## Partial Least Squares
```{r}
set.seed(123)

# fit model
plsModel <- train(PH ~ .,data = train,
                 method = "pls", 
                 tuneLength = 20,
                 trControl = ctrl,
                 # preProc = c("center", "scale"),
                 validation = "CV")

# review performance metrics
pls_pred <- predict(plsModel, test)
pls_results = postResample(pls_pred, test$PH)
pls_results_df = rownames_to_column(data.frame(pls_results), var = "Metric")
colnames(pls_results_df) = c('Metric', 'Score')
knit_table(pls_results_df, 'Partial Least Squares Performance')
```

## Neural Network
```{r, warning=FALSE}
# remove predictors to ensure maximum abs pairwise corr between predictors < 0.75
tooHigh <-findCorrelation(cor(train[, -1]), cutoff = .75)

train_x_nnet <- train[, -tooHigh]
test_x_nnet <- test[, -tooHigh]

nnetGrid <- expand.grid(
  .decay = c(0, 0.01, .1),.size = c(1:5),
  .bag = FALSE)

# fit model
nnetTune <- train(PH ~ .,data = train_x_nnet,
                  method = "avNNet",
                  tuneGrid = nnetGrid,
                  trControl = ctrl,
                  linout = TRUE,
                  trace = FALSE,
                  MaxNWts =  5 * (ncol(train_x_nnet) + 1) + 5 + 1,
                  maxit = 50)

# review performance metrics
nnetPred <- predict(nnetTune, test_x_nnet)
nnet_results = postResample(nnetPred, test_x_nnet$PH)
nnet_results_df = rownames_to_column(data.frame(nnet_results), var = "Metric")
colnames(nnet_results_df) = c('Metric', 'Score')
knit_table(nnet_results_df, 'Neural Network Performance')
```

## MARS
```{r}
marsGrid <- expand.grid(.degree = 1:2, .nprune = 2:38)
# Fix the seed so that the results can be reproduced
set.seed(123)

# fit model
marsTuned <- train(PH ~ .,data = train,
  method = "earth",
  # Explicitly declare the candidate models to test
  tuneGrid = marsGrid,
  trControl = ctrl
  )

# review performance metrics
marsPred <- predict(marsTuned, test)
mars_results = postResample(marsPred, test$PH)
mars_results_df = rownames_to_column(data.frame(mars_results), var = "Metric")
colnames(mars_results_df) = c('Metric', 'Score')
knit_table(mars_results_df, 'MARS Performance')
```

## Random Forest
```{r}
set.seed(123)
rfGrid <- expand.grid(mtry = c(8, 10, 15, 20))

# fit model
rfModel <- train(PH ~ .,data = train,
                 tuneGrid = rfGrid,
                 trControl = ctrl,
                 method = "rf")

# review performance metrics
rf_pred <- predict(rfModel, test)
rf_results = postResample(pred = rf_pred, obs = test$PH)
rf_results_df = rownames_to_column(data.frame(rf_results), var = "Metric")
colnames(rf_results_df) = c('Metric', 'Score')
knit_table(rf_results_df, 'Random Forest Performance')
```

## Boosted Tree
```{r}
gbmGrid <- expand.grid(interaction.depth = seq(1, 7, by = 2),
                       n.trees = seq(100, 1000, by = 50),
                       shrinkage = c(0.01, 0.1),
                       n.minobsinnode = 10)

set.seed(123)

# fit model
gbm <- train(PH ~ .,data = train,
                 method = "gbm",
                 tuneGrid = gbmGrid,
                 verbose = FALSE)

# review performance metrics
gbm_pred <- predict(gbm, test)
gbm_results = postResample(gbm_pred, obs = test$PH)
gbm_results_df = rownames_to_column(data.frame(gbm_results), var = "Metric")
colnames(gbm_results_df) = c('Metric', 'Score')
knit_table(gbm_results_df, 'Boosted Tree Performance')
```

## Cubist
```{r}
set.seed(123)

# fit model
cubist_model <- train(PH ~ .,data = train,
  method = "cubist",
  trControl = ctrl,
  tuneGrid = expand.grid(committees = c(1, 5, 10), neighbors = c(0, 3, 5, 7))
)

# review performance metrics
cubist_pred <- predict(cubist_model, test)
cubist_results = postResample(cubist_pred, test$PH)
cubist_results_df = rownames_to_column(data.frame(cubist_results), var = "Metric")
colnames(cubist_results_df) = c('Metric', 'Score')
knit_table(cubist_results_df, 'Cubist Performance')
```

# Model Evaluation

## Model Comparison
```{r}
# compile all model performance results
final_results = rbind(
  lm_results,
  mars_results,
  pls_results,
  nnet_results,
  rf_results,
  # gbm_results,
  cubist_results
)
# add model name column
final_results = data.frame(cbind(Model = c(
  'Linear Regression',
  'MARS',
  'Partial Least Squares',
  'Neural Network',
  'Random Forest',
  # 'Boosted Tree',
  'Cubist'
  ), final_results))
rownames(final_results) <- NULL
final_results = final_results |> arrange(RMSE)
# output results to table
knit_table(final_results, 'Model Performance Metrics')

# visualize results
final_results = final_results |>
  mutate(RMSE = as.numeric(RMSE)) |>
  mutate(RMSE = round(as.numeric(RMSE), 3))

ggplot(data = final_results, aes(x = reorder(Model, RMSE), y = RMSE)) +
      geom_bar(stat = "identity", fill='#2d3c52') +
      coord_flip() +
      labs(
        title = "Model Comparison: RMSE",
        x = "Model",
        y = "RMSE"
      ) +
      theme_minimal()
```

# Final Model - Random Forest

## Random Forest Predictor Count Performance
```{r}
ggplot(rfModel) |>
  theme_minimal() +
  labs(
    title = "Random Forest # Predictors by RMSE",
    y = 'RMSE',
    x='# Randomly Selected Predictors'
  )
```


## Feature Importance
```{r}
# estimate the importance of each predictor
rf_importance = varImp(rfModel)[1]$importance
rf_importance = rownames_to_column(rf_importance, var = "Predictor")
rf_importance = rf_importance |>
  mutate(Predictor = gsub('`', '', Predictor)) |>
  arrange(desc(Overall))

# visualize results
rf_importance |>
  ggplot(aes(x=reorder(Predictor, Overall), y=Overall)) +
  geom_segment( aes(xend=Predictor, yend=0)) +
  geom_point( size=2, color="#22a298") +
  coord_flip() +
  theme_minimal() +
  labs(
    title = "Random Forest Feature Importance - Top 10",
    y = 'Importance',
    x=''
  )

# output results
knit_table(rf_importance, 'Random Forest Predictor Estimated Importance')
```


# Final Predictions
```{r}
# apply model to final test set
final_pred <- predict(rfModel, test_processed)

# export results to xlsx
final_xslx = final_pred |>
  as.data.frame() 
colnames(final_xslx) = c('PH_Predictions')

write_xlsx(final_xslx, path = "Results/DATA624_Project2_Predictions.xlsx")
```


