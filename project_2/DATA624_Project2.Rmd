...
# train 
train_x <- final_train_data[index, ] %>% select(-PH)
train_y <- final_train_data[index, 'PH']

# test
test_x <- final_train_data[-index, ] %>% select(-PH)
test_y <- final_train_data[-index, 'PH']
```
...
lm_pred <- predict(lmModel,  test_x)
lm_results = postResample(pred = lm_pred, obs = test_y)
lm_results
```
...
plsModel <- train(train_x, train_y, 
                 method = "pls", 
                 tuneLength = 20, trControl = ctrl,
                 preProc = c("center", "scale"),
                 validation = "CV")

pls_pred <- predict(plsModel, test_x)
pls_results = postResample(pls_pred, test_y)
pls_results
```
...
mars_results = postResample(marsPred, test_y)
mars_results
```
...
rf_pred <- predict(rfModel, test_x)
rf_results = postResample(pred = rf_pred, obs = test_y)
rf_results
```
...
set.seed(10)
gbm_chem <- train(train_x, train_y,
                 method = "gbm",
                 tuneGrid = gbmGrid,
                 verbose = FALSE)

gbm_pred <- predict(gbm_chem, test_x)
gbm_results = postResample(pred = gbm_pred, obs = test_y)
gbm_results
```

#### Cubist
```{r}
cubist_model <- train(train_x, train_y,
  method = "cubist",
  trControl = trainControl(method = "cv"),
...
cubist_pred <- predict(cubist_model, test_x)
cubist_results = postResample(pred = cubist_pred, obs = test_y)
cubist_results
```
